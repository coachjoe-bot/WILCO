...
function CoachOverview({athletes,workouts,prs,manualRMs,prescriptions,onOpenAthlete,coach,changeRequests,briefContext,onPrefillProgram,onResolveRequest,onContextWritten,parsingIds,analytics}){
  ...
  const D = useMemo(()=>{
    ...
    const athletePlateau=(wo)=>{
      ...
      // TRIAGE gate: repeating a working weight for a couple of weeks is normal
      // programming, not a coach concern — without these floors the triage
      // flagged most of a healthy roster ("8 athletes need your attention"),
      // including accessories like single-leg RDLs and split squats. A triage
      // slot takes a BENCHMARK lift (getBenchKey — the curated ranked list;
      // accessories return null there) stuck ≥21 days. The digests' lighter
      // has_plateau (any 3 flat entries, any lift) is unchanged.
      if(best && (best.spanDays < 21 || !getBenchKey(normalizeExName(best.lift)))) return null;
      return best;
    };
